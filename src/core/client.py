...
class Twitch:
    def __init__(self, settings: Settings):
        ...
        # when True, CHANNELS_CLEANUP tears down every tracked channel (not just
        # the offline/unwanted ones); set by GAMES_UPDATE and by a CHANNEL_SWITCH
        # re-fetch, consumed and reset by CHANNELS_CLEANUP
    ...
    async def shutdown(self) -> None:
        ...
        # wait out the remainder of half a second (never negative) so aiohttp can
        # finish tearing the session down safely
    ...
    async def _run(self) -> None:
        ...
        while True:
            if self._state is State.IDLE:
                ...
                await self._state_inventory_fetch()
            elif self._state is State.GAMES_UPDATE:
                await self._state_games_update()
            elif self._state is State.CHANNELS_CLEANUP:
                await self._state_channels_cleanup()
            elif self._state is State.CHANNELS_FETCH:
                await self._state_channels_fetch()
            elif self._state is State.CHANNEL_SWITCH:
                await self._state_channel_switch()
    ...
    async def _run_setup(self) -> None:
        """Initialise API clients, websocket, the watch loop and default topics."""
    ...
    async def _state_games_update(self) -> None:
        ...
        next_hour: datetime = datetime.now(UTC) + timedelta(hours=1)
        ...
        await self.notification_service.track_new_campaigns(self.inventory, games_to_watch)

        if self.wanted_games:
            logger.info("Wanted games: %s", ", ".join(game.name for game in self.wanted_games))
        else:
            logger.warning(
    ...
    async def _state_channels_cleanup(self) -> None:
        self.gui.status.update(_.t["gui"]["status"]["cleanup"])
        channels = self.channels
        if not self.wanted_games or self._full_cleanup:
    ...
    async def _state_channels_fetch(self) -> None:
    ...
    async def _state_channel_switch(self) -> None:
        ...
        if new_watching is not None:
            ...
            if should_refetch:
                ...
                if self._state is State.CHANNEL_SWITCH:
                    ...
                    # the directory (CHANNELS_CLEANUP honours _full_cleanup)
                    self._full_cleanup = True
                    self.change_state(State.CHANNELS_CLEANUP)
            else:
    ...
    @property
    def manual_target_game(self) -> Game | None:
        """The game pinned by manual mode, if any (read-only view for services)."""
        return self._manual_target_game
    ...
    # --- Inventory state ownership -------------------------------------------------
    # These methods let InventoryService drive an inventory refresh without reaching
    # into the client's private state directly - the client stays the sole owner of
    # its drop/campaign/maintenance bookkeeping.

    def reset_inventory_state(self) -> None:
        """Clear the per-fetch inventory bookkeeping before it's repopulated."""
        self._drops.clear()
        self.gui.inv.clear()
        self.inventory.clear()
        self._mnt_triggers.clear()

    def register_campaign(self, campaign: DropsCampaign) -> None:
        """Track a freshly-fetched campaign (and its drops) in the inventory."""
        self._drops.update({drop.id: drop for drop in campaign.drops})
        self.inventory.append(campaign)
        self._campaigns[campaign.id] = campaign

    def set_maintenance_triggers(self, triggers: abc.Iterable[datetime]) -> None:
        """Replace the maintenance trigger schedule, dropping any already in the past."""
        self._mnt_triggers.extend(sorted(triggers))
        now = datetime.now(UTC)
        while self._mnt_triggers and self._mnt_triggers[0] <= now:
            self._mnt_triggers.popleft()

    def next_maintenance_trigger(self, upper_bound: datetime) -> datetime:
        """
        Pop and return the earliest queued maintenance trigger at or before
        ``upper_bound``, or ``upper_bound`` itself when none is queued that early.
        """
    ...
    def get_drop(self, drop_id: str) -> TimedDrop | None:
        """Look up a currently-tracked drop by its ID."""
        return self._drops.get(drop_id)

    def restart_maintenance_task(self) -> None:
        """(Re)start the background maintenance task after an inventory refresh."""
        if self._mnt_task is not None and not self._mnt_task.done():
            self._mnt_task.cancel()
        self._mnt_task = asyncio.create_task(self._maintenance_service.run_maintenance_task())

    def is_exiting(self) -> bool:
        """Whether the miner has been asked to exit (checked during long fetches)."""
