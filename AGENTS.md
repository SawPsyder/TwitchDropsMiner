...
- `StreamSelector` (services/stream_selector.py) builds the actual wanted-items queue from that two-tier list; if it comes up empty (the watched games have nothing earnable right now) and `settings.idle_behavior["mine_all_when_idle"]` is enabled, it falls back to every game with an active campaign instead of leaving the queue empty. Each queue entry is tagged with a `source` of `"manual"`, `"auto"`, or `"idle"` (shown as a badge in the web GUI's Wanted Drop Queue). `StreamSelector.get_unlinked_auto_tracked_tree()` surfaces a separate list of manually-watched and auto-tracked games (Steam, Ubisoft, ...) that have at least one campaign whose Twitch account isn't linked yet - manual games first, then auto ones not already on the manual list (no duplicates), each tagged `"manual"`/`"auto"`; this bypasses the usual eligible/can-earn-within gate so unlinked campaigns without a badge/emote reward (which are never "eligible") still show up - each campaign dict in both trees now carries `linked`/`link_url` fields. This is rendered as its own horizontal, unordered "Tracked Games Awaiting Link" panel below the Wanted Drop Queue in the web GUI, with a single Link/Refresh Status button per game card (broadcast via the `unlinked_auto_items_update` Socket.IO event and the `unlinked_auto_items` key in `initial_state`)
...
The application sends periodic "watch" payloads by POSTing to the Spade tracking endpoint
(extracted from the channel page / settings.js; `beacon_url` is preferred over `spade_url`
since spade.twitch.tv is on common ad-block DNS lists):

- Payload contains base64-encoded minute-watched events with channel/broadcast IDs
- NOTE: the GQL `sendSpadeEvents` mutation stopped counting watch time (~July 2026) -
  it still returns statusCode 204, but drop progress does not advance
...
Persisted operations are defined in `src/config/operations.py` as `GQL_OPERATIONS`; raw GraphQL payloads (e.g. the unused `sendSpadeEvents` variant) use `GQLQuery`:
